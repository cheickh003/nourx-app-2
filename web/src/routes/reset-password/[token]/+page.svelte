<script>
	...
	onMount(() => {
		...
		// Get user type from URL search params if provided
	...
	async function handlePasswordReset() {
		...
		try {
			...
			success = true;
			toastStore.success('Password reset successful! You can now sign in with your new password.');
			...
			if (err.status === 400) {
				...
				} else if (err.data?.validationErrors) {
					const validationError = Object.values(err.data.validationErrors)[0];
					error = validationError || 'Password does not meet requirements.';
				} else {
					error = err.data?.error || 'Password reset failed. Please try again.';
				}
	...
	function goToForgotPassword() {
		goto('/forgot-password' + (userType === 'admin' ? '?type=admin' : ''));
	}
...
<svelte:head>
	<title>Reset Password - NOURX</title>
		...
		<Card class="p-8">
			...
			{:else if tokenValid === false || error}
				<Alert color="red" class="mb-6">
					<span class="font-medium">Reset Error:</span> {error || 'Invalid reset link'}
				</Alert>
			{/if}
			...
			{#if tokenValid && !success}
				<div class="mb-6 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
					<div class="flex items-start space-x-3">
						...
						<div>
							<p class="text-sm font-medium text-blue-900 dark:text-blue-100">Create New Password</p>
