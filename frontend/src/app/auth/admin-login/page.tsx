...
export default function AdminLoginPage() {
  ...
  const onSubmit = async (data: { email: string; password: string; rememberMe?: boolean }) => {
    ...
    try {
      ...
      if (userType === 'client') {
        // Si un client s'est connecté sur la page admin, diriger vers le portail client
        const safeRedirect = redirectParam && redirectParam.startsWith('/client') ? redirectParam : '/client/dashboard';
        router.push(safeRedirect);
        return;
      }

      // Par défaut, portail admin
      ...
      <Card className="w-full max-w-md">
        <CardHeader className="space-y-1">
          ...
          <CardDescription className="text-center">
            Connectez-vous à votre espace d'administration Nourx
          </CardDescription>
        </CardHeader>
        <CardContent>
